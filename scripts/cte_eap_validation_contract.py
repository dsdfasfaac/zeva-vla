"""Pure, cache-independent ZeVA CTE/BIT/EAP validation controls."""
...
def enumerate_decisions(records, tasks):
    """Derive the H15 grid from raw episode lengths, never cached errors."""
    result = []
    for index, record in enumerate(records):
        ...
        for timestep, frame in enumerate([*starts, starts[-1] + 15]):
            result.append(
                {
                    "sample_id": f"{record_id}@{frame}",
                    "record_id": record_id,
                    "record_index": index,
                    "task": task,
                    "frame": frame,
                    "cache_index": timestep,
                }
            )
    ids = [row["sample_id"] for row in result]
    if not result or len(ids) != len(set(ids)) or {row["task"] for row in result} != set(tasks):
...
def within_task_permutation(decisions, seed=20260918):
    ...
    for task, ids in sorted(groups.items()):
        ...
        output.update(zip(ids, ids[1:] + ids[:1]))  # noqa: B905
    return output


def decision_noise_seed(sample_id):
    """Return a stable noise seed across processes, machines and iteration order."""
    return int.from_bytes(
        hashlib.sha256(f"zeva-validation5-v1:{sample_id}".encode()).digest()[:8], "big"
    ) % (2**63 - 1)
...
def check_cache_coverage(decisions, cache):
    ...
    for record_id, frames in expected.items():
        ...
        if tuple(entry["phase"].shape) != (len(frames), 256):
            raise ValueError(f"Bad recurrent phase-cache shape: {record_id}")
        if tuple(entry["effect"].shape) != (len(frames), 256):
            raise ValueError(f"Bad recurrent effect-cache shape: {record_id}")
