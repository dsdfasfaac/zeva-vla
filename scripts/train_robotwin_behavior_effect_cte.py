"""Train the BehaviorVLA-aligned ZeVA CTE + effect objective on train95 only.

Example: python scripts/train_robotwin_behavior_effect_cte.py --dataset-root ...
         --save-dir ... --task-subset configs/robotwin_zeva_advantage10.json
The fixed endpoint is epoch80; validation never selects from formal test labels.
"""
...
from openpi.zeva.behavior_effect import SCHEMA, UPSTREAM_COMMIT, ZevaCTE, ZevaCTEConfig, cte_loss
...
class Args:
    ...
    dataset_identity_report: str | None = None
    source_task_identity_report: str | None = None
...
def main(args: Args):
    if args.batch_size != 8 or args.epochs != 80 or args.effect_weight != .2:
        raise ValueError("This preregistered candidate fixes batch8, epoch80 and effect weight0.2.")
    ...
    if args.dataset_identity_report:
        reference = Path("/mnt/100T/users/dingxin/VLA/zeva-runs/robotwin-v5-h15-tasklang/dataset-identity-aigc29-stage2-resume-20260912.json")
        if sha(reference) != "3ba76d89295564cabb735aecc47ddb55cb039fe21f4072138a14d1f02fb20666":
            raise ValueError("Frozen source dataset identity report SHA changed.")
        replica = json.loads(Path(args.dataset_identity_report).read_text())
        source = json.loads(reference.read_text())
        if replica["schema"] == "zeva-behavior-effect-task-data-identity-v1":
            if not args.source_task_identity_report:
                raise ValueError("Task-scoped replica requires independent canonical-host task report.")
            task_source = json.loads(Path(args.source_task_identity_report).read_text())
            if task_source["adapter_sha256"] != source["adapter_sha256"]:
                raise ValueError("Canonical task report does not reference the frozen source adapter.")
            expected_components = {k:v for k,v in source["components"].items() if k != "source"}
            if replica["components"] != expected_components or task_source["components"] != expected_components:
                raise ValueError("Action/state/statistics differ from the frozen source.")
            for key in ("tasks", "source_tasks", "semantic_adapter"):
                if replica[key] != task_source[key]:
                    raise ValueError(f"Independent ten-task source/replica comparison failed: {key}")
            if replica["tasks"] != json.loads(Path(args.task_subset).read_text())["task_names"]:
                raise ValueError("Identity proof covers a different task subset.")
        elif replica["components"] != source["components"] or replica["semantic_adapter"] != source["semantic_adapter"]:
            raise ValueError("Dataset replica differs from the frozen source, including videos.")
        if replica["adapter_sha256"] != sha(manifest):
            raise ValueError("Replica identity report belongs to a different adapter.")
        identity_evidence = {"reference":str(reference), "reference_sha256":sha(reference),
                             "replica":args.dataset_identity_report, "replica_sha256":sha(args.dataset_identity_report)}
        if args.source_task_identity_report:
            identity_evidence["task_source_sha256"] = sha(args.source_task_identity_report)
    elif not args.smoke_steps:
        raise ValueError("Full Stage1 requires a complete dataset identity report, including videos.")
    ...
    identity = {"schema": SCHEMA, "upstream_commit": UPSTREAM_COMMIT,
                ...
                "dataset_identity": identity_evidence,
                "runtime": {"torch":torch.__version__, "cuda":torch.version.cuda, "gpu":torch.cuda.get_device_name()},
                "source_sha256": {"trainer": sha(__file__), "model": sha(Path(__file__).parents[1]/"src/openpi/zeva/behavior_effect.py")},
                "normalization": "baseline-mean-std", "decoder": "torchcodec",
                "selection": "fixed-epoch80; validation5-only gate; no formal labels",
    ...
    if args.resume:
        ...
        if not checkpoint["manifest"]["promotable"]:
            raise ValueError("A smoke checkpoint cannot resume a formal train run.")
        ...
        raise ValueError("Fresh run requires an empty output directory; preserving existing results.")
    ...
    (output / "COMPLETED").write_text("epoch80 completed; apply validation gate before Stage2\n")
