...
episode at times ``[0, t)``.  The validated Parent policy is frozen bit-for-bit;
...
def main(args: Args) -> None:
    ...
    if args.max_entries != 64 or args.pim_lr != 5e-5:
        raise ValueError("Episode-PIM capacity64 and LR5e-5 are preregistered.")
    ...
    manifest = {
        ...
        "selection": "fixed-step2000; validation-only before new disjoint single-attempt closed-loop",
        "promotable": not args.smoke_test,
        "post_formal_candidate": True,
        ...
        for item in loader:
            ...
            if step % 500 == 0 or (args.smoke_test and step == args.steps):
                ...
                if accelerator.is_main_process:
                    ...
                    torch.save({"identity": model.identity, "eap": model.pbd.state_dict()}, folder / "zeva_adapter.pth")
                    torch.save(
    ...
    if accelerator.is_main_process:
        (output / "COMPLETED").write_text(
            "nonpromotable runtime smoke\n" if args.smoke_test else
            "fixed 2000 steps; validation and new disjoint single-attempt evaluation required\n"
        )
