...
export PYTHONPATH="$zeva_deps:$system_site:$runtime/h100-extra-deps:$runtime/lerobot-main-deps-py311-v1:$runtime/lerobot-overlay-v2:$EGOSCALE_LEROBOT_SOURCE:$runtime/src:$zeva_root/src${PYTHONPATH:+:$PYTHONPATH}"
...
exec "$python_bin" -m accelerate.commands.launch \
  ...
  --num_processes 8 \
  --mixed_precision no \
  "$zeva_root/scripts/train_robotwin_zeva_adapter.py" \
  "$@"
