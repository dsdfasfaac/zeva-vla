...
def train_loop(config: _config.TrainConfig):
    ...
    if not isinstance(config.model, openpi.models.pi0_config.Pi0Config):
        ...
        model_cfg = dataclasses.replace(config.model, is_training=True)
    ...
    #         if "time_mlp" not in k
    #     }
        
    
    #     skipped_keys = [k for k in state_dict.keys() if "time_mlp" in k]
    #     logging.info(f"Skipped {len(skipped_keys)} 'time_mlp' keys to avoid shape mismatch.")
    ...
    #     logging.info(f"Missing keys (Initialized randomly): {len(missing_keys)}")
    
    #     # logging.info(f"First 10 missing keys: {missing_keys[:10]}")
