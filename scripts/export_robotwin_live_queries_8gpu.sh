...
export PYTHONPATH="$zeva_deps:$system_site:$runtime/h100-extra-deps:$runtime/lerobot-main-deps-py311-v1:$runtime/lerobot-overlay-v2:$runtime/lerobot-main-py311-v1/src:$runtime/src:$zeva_root/src:$zeva_root${PYTHONPATH:+:$PYTHONPATH}"
...
exec "$python_bin" -m accelerate.commands.launch \
  ...
  --num_processes 8 \
  --mixed_precision no \
  "$zeva_root/scripts/export_robotwin_live_queries.py" \
  "$@"
