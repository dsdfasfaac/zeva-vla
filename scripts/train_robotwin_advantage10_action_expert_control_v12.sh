...
# v12 causal control: only the PI0.5 action expert updates.  The immutable
# untouched best-v1 action path is used for same-noise per-example PBD on the
# configured preservation interval.  ZeVA modules are loaded only so the
# shared Stage 2 data/runtime contract remains identical; they are frozen and
...
run_root=${RUN_ROOT:-/mnt/100T/users/dingxin/VLA/zeva-runs/robotwin-v5-h15-tasklang/advantage10-action-expert-control-v12}

smoke=${V12_SMOKE:-0}
if [[ "$smoke" == 1 || "$smoke" == true ]]; then
  output=${V12_SMOKE_OUTPUT:-$run_root/action_expert_control-smoke}
  steps=1
  save_freq=1
  eval_batches=${EVAL_BATCHES:-1}
  num_workers=0
else
  output=${V12_OUTPUT:-$run_root/action_expert_control}
  steps=${STEPS:-2000}
...
if [[ "$actual_foundation_sha256" != "$foundation_sha256" ]]; then
  echo "unexpected untouched best-v1 foundation hash: $actual_foundation_sha256" >&2
  exit 4
fi

if [[ -e "$output/COMPLETE" ]]; then
  echo "v12 action-expert control is already complete: $output" >&2
  exit 0
fi
if [[ -e "$output/manifest.json" && -z "$resume_checkpoint" ]]; then
  echo "refusing to overwrite existing v12 run: $output" >&2
  exit 3
fi
...
mkdir -p "$output"
printf '%s\n' \
  "starting action-expert control v12 $(date --iso-8601=seconds) " \
  "steps=$steps save_freq=$save_freq smoke=$smoke resume=${resume_checkpoint:-none}" \
...
bash "$zeva_root/scripts/train_robotwin_stage2_8gpu.sh" \
  --training-variant action_expert_control \
...
printf '%s\n' "completed action-expert control v12 $(date --iso-8601=seconds)" \
  | tee "$output/COMPLETE"
