...
# One-shot seed-10000 final.  It is unreachable until both disjoint v8
# validation splits pass.  The untouched best-v1 anchor is evaluated on the
# same frozen seeds, so the trained Base must retain normal PI0.5 ability and
# ZeVA must strictly beat that contemporaneous Base.
...
if summary.get("schema") != "zeva-robotwin-v8-action-expert-closed-loop-validation-summary-v1":
    raise SystemExit("unexpected v8 validation-summary schema")
if summary.get("accepted_for_fresh_final_test") is not True:
    raise SystemExit("fresh final is forbidden because v8 validation did not pass")
if plan.get("schema") != "zeva-robotwin-v8-action-expert-closed-loop-validation-plan-v1":
    raise SystemExit("unexpected v8 validation-plan schema")
...
print(plan["base_checkpoint"])
print(plan["zeva_checkpoint"])
print(plan["shared_checkpoint_step"])
print(plan["base_model_sha256"])
print(plan["zeva_model_sha256"])
print(plan["zeva_adapter_sha256"])
print(plan["base_config"])
print(plan["zeva_config"])
print(plan["base_config_sha256"])
print(plan["zeva_config_sha256"])
PY
)
...
base_config=${selected[6]}
zeva_config=${selected[7]}
base_config_sha256=${selected[8]}
zeva_config_sha256=${selected[9]}
...
python3 - "$final_root/final_plan.json" "$base_checkpoint" "$zeva_checkpoint" \
  "$checkpoint_step" "$base_model_sha256" "$zeva_model_sha256" "$zeva_adapter_sha256" \
  "$base_config" "$zeva_config" "$anchor_config" <<'PY'
import hashlib
import json
import os
import sys
...
(destination, base_checkpoint, zeva_checkpoint, step, base_hash, zeva_hash,
 adapter_hash, base_config, zeva_config, anchor_config) = sys.argv[1:]
payload = {
    "schema": "zeva-robotwin-v8-action-expert-fresh-final-plan-v1",
...
if path.is_file() and json.loads(path.read_text()) != payload:
    raise RuntimeError("existing v8 final plan differs; refusing mutation")
if not path.is_file():
...
if [[ ! -s "$final_root/paired_report.json" ]]; then
  env MODEL_HOST="$model_host" MODEL_IP="$model_ip" \
    ...
    BASELINE_LABEL="action-expert-v8-base-step-$checkpoint_step-fresh-final" \
    ANCHOR_LABEL="untouched-best-v1-same-seed-anchor" \
    ZEVA_LABEL="action-expert-v8-zeva-step-$checkpoint_step-fresh-final" \
...
print("v8 final seed manifest is disjoint from both validation splits")
PY
