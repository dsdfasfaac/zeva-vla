#!/usr/bin/env bash
# Run one frozen multi-attempt development condition against the audited Base set.
set -euo pipefail

condition=${1:-}
[[ "$condition" == parent || "$condition" == pim ]] || {
  echo 'usage: launch_pim_multiattempt_condition.sh parent|pim' >&2; exit 2;
}
zeva_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
run=/mnt/100T/users/dingxin/VLA/zeva-runs/robotwin-pim-20260919
base_root=$run/eval/development-multiattempt-base-seed2000000
...
if [[ "$condition" == parent ]]; then
  ...
  reset_scope=episode
  label=cte-bit-eap-parent-step5000-multiattempt-development
  output=$run/eval/development-multiattempt-parent-seed2000000
  expected_uuids=(
  ...
  reset_scope=attempt
  label=cte-bit-pim-eap-step2000-multiattempt-development
  output=$run/eval/development-multiattempt-pim-seed2000000
  expected_uuids=(
...
[[ $(hostname -s) == "$expected_host" ]] || { echo "$condition launch is pinned to $expected_host" >&2; exit 2; }
for required in "$base_root/baseline/report.json" "$base_root/baseline/state.json" \
  "$base_root/seed_manifest.json" "$base_root/development_seed_audit.json" "$formal_seed" \
  "$task_manifest" "$old_formal" "$config" "$loader_dir/libvulkan.so.1" "$loader_dir/libEGL.so.1"; do
  [[ -s "$required" ]] || { echo "Missing frozen input: $required" >&2; exit 2; }
done
python3 - "$base_root/baseline/report.json" "$base_root/development_seed_audit.json" <<'PY'
import json, pathlib, sys
report=json.loads(pathlib.Path(sys.argv[1]).read_text())
audit=json.loads(pathlib.Path(sys.argv[2]).read_text())
if report.get("total_episodes") != 80 or report.get("total_successes") != 62:
    raise SystemExit("Frozen Base result differs from audited 62/80")
if audit.get("status") != "PASS" or audit.get("overlap_with_formal") != 0 or audit.get("overlap_with_prior_development") != 0:
...
export BASE_PORT=$base_port EPISODES=8 ABSOLUTE_START_SEED=2000000
export MAX_POLICY_ATTEMPTS=4 BASELINE_ATTEMPT_RESET_SCOPE=episode ZEVA_ATTEMPT_RESET_SCOPE=$reset_scope
...
export PRECOMPUTED_BASELINE_EXPECTED_SUCCESSES=62
