...
python3 - "$eval_root/pipeline_state.json" "$eval_root/split-d/paired_report.json" \
  "$failed_split/baseline/progress" <<'PY'
import json
import sys
from pathlib import Path

state_path, split_d_report, progress_root = map(Path, sys.argv[1:])
...
if report.get("total_paired_episodes") != 80:
    raise SystemExit("split-d did not complete all 80 paired episodes")
completed = 0
...
print("recovery preconditions passed: split-d complete, split-c episodes=0")
PY
...
payload = {
    ...
    "preserved_completed_split": "split-d",
