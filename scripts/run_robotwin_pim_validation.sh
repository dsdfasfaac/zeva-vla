...
[[ $(hostname -s) == aigc28 ]] || { echo 'PIM validation launcher is audited for aigc28' >&2; exit 2; }
...
output=$run/validation5-pim-step2000-8shard
checkpoint=$run/stage2-cte-eap-pim-parent61-fixed2000-8gpu/002000
...
for path in "$checkpoint/COMPLETE" "$parent/COMPLETE" "$cte" "$cte_artifacts" \
            "$pim_artifacts" "$retrieval" "$foundation/model.safetensors" "$expected"; do
...
for gpu in "${expected_gpus[@]}"; do
  ...
  [[ $used -lt 1024 && $util -le 5 ]] || {
    echo "GPU $gpu is not safely idle; refusing to oversubscribe" >&2; exit 2;
  }
done

mkdir "$output"
common=("$zeva_root/scripts/report_robotwin_pim_validation.py"
  ...
  --foundation-checkpoint "$foundation" --parent-checkpoint "$parent"
  --checkpoint "$checkpoint" --expected-decisions "$expected" --output-dir "$output"
  --tasks "$zeva_root/configs/robotwin_zeva_advantage10.json" --handoff "$handoff"
  --num-shards 8)
...
[[ $status -eq 0 ]] || { echo 'One or more PIM validation shards failed' >&2; exit 1; }

/usr/bin/python3 "${common[@]}" --shard-index 0 --merge >"$output/merge.log" 2>&1
printf 'complete frozen validation5 report\n' >"$output/COMPLETE"
