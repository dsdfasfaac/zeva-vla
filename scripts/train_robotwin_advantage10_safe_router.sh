...
# Add task/phase-routed ZeVA residuals to the untouched RoboTwin PI0.5.  The
# foundation and Stage1 artifacts remain frozen; protection uses a matched
# per-example PI teacher so regressions cannot cancel across tasks.
...
output=${RUN_ROOT:-/mnt/100T/users/dingxin/VLA/zeva-runs/robotwin-v5-h15-tasklang/advantage10-safe-router-v3}/zeva

mkdir -p "$output"
printf '%s\n' "starting safe-router ZeVA $(date --iso-8601=seconds)" | tee -a "$output/STARTED"
...
bash "$zeva_root/scripts/train_robotwin_stage2_8gpu.sh" \
  ...
  --gate-regularization-weight "${GATE_REGULARIZATION_WEIGHT:-0.01}" \
...
printf '%s\n' "completed safe-router ZeVA $(date --iso-8601=seconds)" | tee -a "$output/COMPLETE"
