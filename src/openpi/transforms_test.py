...
@pytest.mark.parametrize(("frame_index", "expected_mask"), [(0, False), (7, True)])
def test_extract_previous_action(frame_index, expected_mask):
    actions = np.arange(12, dtype=np.float32).reshape(4, 3)
    transform = _transforms.ExtractPreviousAction()

    result = transform({"actions": actions, "frame_index": np.asarray(frame_index)})

    np.testing.assert_array_equal(result["previous_action"], actions[0])
    np.testing.assert_array_equal(result["actions"], actions[1:])
    assert result["has_previous_action"].item() is expected_mask


def test_extract_previous_action_rejects_missing_history():
    transform = _transforms.ExtractPreviousAction()

    with pytest.raises(ValueError, match=r"horizon \+ 1"):
        transform({"actions": np.ones((1, 7)), "frame_index": np.asarray(0)})
