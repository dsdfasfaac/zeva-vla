...
**最终门槛结论（2026-09-16）：** 1000步NLL梯度隔离候选对训练Base1000的同batch16/full/seed1000诊断已完成。候选H15 on=0.0101641398、off=0.0101662474，残差内部改善0.02073%；但同预算Base1000=0.0100931218，候选仍差0.70363%。自身off也比Base差0.72451%，说明主要缺口是action expert更新后的基础路径漂移，新残差只扳回很小一部分。预注册门槛失败，不启动正式闭环、不延长该路由训练。正式成功率仍为Base55%/ZeVA55%。[完整证据](ZTEV2_NLL_ROUTING_BUDGET_20260916.md)。

**最新结果（2026-09-16 12:35）：** NLL梯度隔离候选已完成1000/1000步及5874决策全量验证，H15 residual-on=0.0101641398、off=0.0101662474，开启残差内部改善0.02073%。这只通过自身off对照，不等于超过同预算训练Base。已启动候选对Base1000的batch16/full/seed1000同噪声只读诊断，PID1287584；结果出来前不进入正式闭环。仍无新成功率。[原始证据和停止条件](ZTEV2_NLL_ROUTING_BUDGET_20260916.md)。
...
**后续机制实验（2026-09-15）：** 固定100步、初始gate 0.01与0.10两臂，均从Base004500新建零残差与optimizer；其余冻结范围、LR、global256、Gaussian NLL与H50/H15不变。aigc29两臂真实权重预检均exit0，gate001已完成100步及全5874决策验证；完整性字段因CLI显式上限语义拦住了编排，已核对实际全覆盖且保留原始报告，仅修正后续诊断调用并启动gate010。尚无第二臂结果。只用训练/验证数据检验初始化强度是否改善动作条件信息利用，不用正式测试标签调倍率；这不是新的匹配普通Base成功率交付。[固定设计与执行记录](ZTEV2_GATE_MECHANISM_20260915.md)。

**最终闭环结果（2026-09-15）：Base=110/200（55.0%）、ZeVA=110/200（55.0%）、Anchor=104/200（52.0%）。ZeVA未获得总体提升，Base低于57%参考线，验收未通过。** 配对差值0个百分点，95% CI为[−7.5,+8.0]个百分点，McNemar p=1.0。评测已完成，不再写为等待Anchor/ZeVA；仅迁移运行节点，方法、权重、任务、H50/H15和评测协议未改变。完整十任务表、证据与限制见[本轮报告](ROBOTWIN_FIXED_ANCHOR_PAIRED_RESULTS_20260915.md)。

**训练后诊断（2026-09-14）：固定teacher新轮的1000步训练与5874决策终检均已完成。** 同噪声H15误差：原Base004500=0.01018073、新Base001000=0.01003096、新ZeVA001000 residual-off=0.01014233、on=0.01014354。ZeVA虽优于训练起点，却比同预算Base差1.1224%，残差开启也略差于关闭；没有证明ZTE增量收益，不能把固定teacher包装成已有效的方法。预定末步的配对闭环现已完成，结果见上段；未加训或挑checkpoint/倍率。下列较早执行进度为历史记录，以本页顶部及[最新诊断](ZTEV2_POST_EVAL_DIAGNOSIS_20260912.md)为准。
