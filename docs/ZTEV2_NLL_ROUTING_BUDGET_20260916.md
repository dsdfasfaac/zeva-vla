...
## 2026-09-16：预注册门槛失败，不进入正式闭环

候选对同预算训练Base1000的最终只读诊断已完成。全368批/5874决策，batch16、seed1000、H50输出/H15执行，样本顺序SHA与两份先前报告完全一致；新候选、Base1000模型哈希和共享冻结权重校验均通过。
...
残差on比自身off低0.02073%，通过第一个条件；但比训练Base高0.70363%，第二个条件失败，`promote_formal=false`。而off路径本身比Base高0.72451%，残差仅恢复约0.0209个百分点的相对误差，不足以弥补action expert训练漂移。[原始匹配报告](results/robotwin-nll-routing-full-20260916/nll-detached-full1000-vs-base1000-b16-a29.json)。

按预注册停止条件：

- 不给这条NLL梯度隔离路线继续加步数；
- 不将该checkpoint送入10任务正式闭环；
- 不用Base004500取代同预算Base1000声称通过；
- 下一个假设必须直接约束/消除action expert基础路径漂移，而不是重复当前路由或基于测试成功标签调参。
