...
> 状态：2026-09-11 的研究设计与诊断假设归档。按用户后续要求，本文的全套表示/attribution 门槛不再作为接入 PI 或启动 Stage2 的强制前置条件；当前执行口径以 README 和方法文档顶部为准。数据/权重正确性、无未来或测试泄漏、H50/H15 对齐仍必须成立；最终目标是正常 Base 与 ZeVA 的配对闭环成功率，而不是完成全部独立表征研究。
