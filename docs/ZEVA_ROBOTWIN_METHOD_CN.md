...
**后续机制实验（2026-09-15）：** 固定100步、初始gate 0.01与0.10两臂，均从Base004500新建零残差与optimizer；其余冻结范围、LR、global256、Gaussian NLL与H50/H15不变。aigc29两臂真实权重预检均exit0，训练控制器已启动，尚未确认optimizer步数。只用训练/验证数据检验初始化强度是否改善动作条件信息利用，不用正式测试标签调倍率；这不是新的匹配普通Base成功率交付。[固定设计与执行记录](ZTEV2_GATE_MECHANISM_20260915.md)。
