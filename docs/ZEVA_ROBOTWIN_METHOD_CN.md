...
**当前方法与执行状态（2026-09-16）：** 不再延长已失败的 NLL-only 路由。本轮在原有 action-expert 侧双残差/Gaussian prior 上采用默认关闭的梯度分路：AE 只用同批同噪声的当前 student residual-off flow 梯度，ZeVA 模块只用 residual-on 目标，分别裁剪，避免新模块目标改变普通 Base 的 AE 更新方向。Stage1 ZTE/bank/检索及 VLM 仍冻结，保留真实 task-language/H15 recurrent state、H50 输出/H15 执行、global256、AE `5e-6`/新模块 `5e-5`。真实 PI 权重的同批梯度检查与四卡单步训练已通过；独立固定1000步候选在 aigc29 运行，尚无终检或新成功率。只有末步 H15 同时优于自身 off 和同预算普通训练 Base1000，才进入十任务配对闭环；不能凭预检或旧成功率宣称达标。[新路线、证据与停止条件](ZTEV2_GRADIENT_ROUTE_20260916.md)。
