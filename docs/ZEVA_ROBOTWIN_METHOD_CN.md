...
诊断口径补充：当前 step4096 的四项已测 Stage1 辅助指标均达到配置参考线；不能把固定的 `probe_gate_passed=false` 解释成实测不合格。正在补默认关闭的 H15/注入幅度/对照身份验证观测，尚未修改训练目标或启动新训练。[已测事实、未测假设与下一步边界](ZTEV2_POST_EVAL_DIAGNOSIS_20260912.md)。
