...
诊断口径补充：当前 step4096 的四项已测 Stage1 辅助指标均达到配置参考线；不能把固定的 `probe_gate_passed=false` 解释成实测不合格。全5874决策诊断已完成：H15双残差相对关闭残差仅改善0.0591%，相对固定Base仅改善0.0130%；prior重建幅度约为context的1/231。正在准备验证集分支消融以判断该改注入还是信息内容；这些不是成功率，未启动新训练。[完整数值、限制与下一步](ZTEV2_POST_EVAL_DIAGNOSIS_20260912.md)。
