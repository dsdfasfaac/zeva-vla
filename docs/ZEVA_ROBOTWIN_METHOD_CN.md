...
诊断口径补充：当前 step4096 的四项已测 Stage1 辅助指标均达到配置参考线；不能把固定的 `probe_gate_passed=false` 解释成实测不合格。全5874决策诊断已完成：H15双残差相对关闭残差仅改善0.0591%，相对固定Base仅改善0.0130%；prior重建幅度约为context的1/231。三组验证集分支消融和一组同机原始对照已在aigc29 GPU0–3启动，尚无完整消融结果；这些不是成功率，未启动新训练。[完整数值、限制与下一步](ZTEV2_POST_EVAL_DIAGNOSIS_20260912.md)。

最新完整结果（2026-09-12）：Base109/200=54.5%，ZeVA110/200=55.0%，Anchor101/200=50.5%。600个视频及配对seed/指令校验通过；ZeVA仅多成功1次，paired 95% CI为[-7.5,+8.5] pp，且Base未过57%参考线，故验收未通过。当前训练与正式评测均已完成，进入证据驱动诊断，未启动新训练；以下早期运行状态均为历史快照。[完整十任务表与限制](ROBOTWIN_ZTEV2_PAIRED_RESULTS_20260912.md)。
