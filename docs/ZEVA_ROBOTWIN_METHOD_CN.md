...
诊断口径补充：当前 step4096 的四项已测 Stage1 辅助指标均达到配置参考线；不能把固定的 `probe_gate_passed=false` 解释成实测不合格。真实005000的两条验证决策已通过H50重放等价性检查（差值0）；2026-09-13 01:32正在加载全5874决策的H15/固定Base只读诊断，尚无完整数值，未修改训练目标或启动新训练。[已测事实、未测假设与下一步边界](ZTEV2_POST_EVAL_DIAGNOSIS_20260912.md)。
