...
最新状态（2026-09-16）：下文的100步候选已完成，但没有证明残差收益；H15开启残差比关闭差0.01686%。独立预声明的1000步预算检验先遇aigc24模型加载CUDA timeout，迁移aigc29后又因长TMPDIR造成DataLoader socket路径超限。现已完成短路径的真实多进程检查，以同配置重派aigc29，PID1199343，确认至少完成2个optimizer steps；不是恢复短轮或将失败改称通过。batch16/full的既有训练Base/旧ZeVA对照已完成。[末步结果、固定预算与迁移证据](ZTEV2_NLL_ROUTING_BUDGET_20260916.md)。下文启动段落保留为历史记录。
